...
Plugins can be included individually (using Bootstrap's individual `*.js` files), or all at once using `app-bootstrap.js` or the minified `app-bootstrap.js` (don't include both).
...
All Bootstrap's JavaScript files depend on `util.js` and it has to be included alongside the other JavaScript files. If you're using the compiled (or minified) `app-bootstrap.js`, there is no need to include this—it's already there.
